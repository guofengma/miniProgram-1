...
Page({
      ...
      success: function (res) {
        ...
        if (code == 0) {
          ...
          for (var i = 0; i < list.length; i++) {
            selectList.push({ "id": list[i].fabricId, "name": list[i].fabric.fabricDescribe, "num": list[i].fabric.fabircNum, "fabricType": list[i].fabricTypeId })
          }
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data.code == 0) {
          ...
          if (list.length==1){
            place_list.push({
              "shelvesId": selectList[0].value,
              "shelvesIndex": "0",
              "shelvesName": selectList[0].name.split(" ")[0],
              "unit": selectList[0].unit,
              

            })
            that.getBatch(token,0)
          }
      ...
      success: function (res) {
        ...
        if (res.data.code == 0) {
          ...
          var selectList = [],
              index=""
          for (var i = 0; i < list.length; i++) {
            selectList.push({ "id": list[i].id, "name": list[i].batch, "count": list[i].num,   })
          }
          if (list.length==1){//如果只有一条数据
            place_list[currIndex].batch = { "index": "0", "selectList": selectList, 'id': list[0].id, 'name': list[0].batch, "count": list[0].num, "num": list[0].num}
          }else{
            place_list[currIndex].batch = { "index": "", "selectList": selectList, 'id': "", 'name': "", "count": "" ,"num":""}
          }
  ...
  bindBatchPickerChange: function (e) {
    ...
    place_list[curIndex].batch.num = selectList[val].count;
  ...
  changeValue: function (e) {
    var dataset = e.target.dataset,
      ...
      val = e.detail.value,
      place_list = this.data.place_list;
    place_list[index].batch[vName] = val;
    this.setData({
      place_list: place_list
    })
    console.log(place_list)
  ...
  formSubmit: function (e) {
    ...
    for (var i = 0; i < l; i++) {
      if (place_list[i].shelvesId == "") {
        warn = "请选择点库位置"
        flag = false;
      } else if (place_list[i].batch.num == "") {
        warn = "请输入点库数量"
        flag = false;
  ...
  postData: function (data, place_list) {
    var that = this;
    var url = app.globalData.servsers + "rz/warehouse/check";//出库接口地址
    console.log(data)
    ...
    for (var i = 0,l = place_list.length; i < l; i++) {
      ...
      var shelves = {
        ...
        "realNum": parseFloat(place_list[i].batch.num)//实际数量
      }
          ...
          wx.showToast({
            ...
            success: function () {
              ...
              for (var i = 0, l = place_list.length;i<l;i++){
                ...
                warehouseList.push({ placeName: place_list[i].shelvesName, count: place_list[i].batch.num })
              }
                console.log(warehouseList)

                // 成功后跳转到success页面
                var warehouseInfo = { label: "点库", unit: place_list[0].unit, num: data.num, numlabel: "原料", url: "../warehouseCheck/warehouseCheck", warehouse: warehouseList }

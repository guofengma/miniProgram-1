...
Page({
  ...
  goDetail:function(e){
    ...
    wx.navigateTo({
      url: '../materialReturnDetail/materialReturnDetail?id='+id,
    })
  },
  ...
  dataProcessing: function (data, page) {
    ...
    for (var i = 0; i < dataList.length; i++) {
      ...
      var T = new Date(predictBackDate); // 将指定日期转换为标准日期格式。Fri Dec 08 2017 20:05:30 GMT+0800 (中国标准时间)
      console.log(T)
      predictBackDate = T.getTime() // 将转换后的标准日期转换为时间戳。
      var CurentTime = new Date();//当前时间
      CurentTime = CurentTime.getTime(CurentTime)
      if (CurentTime > predictBackDate){
        dataList[i].statusClass="red"
      }
      dataList[i].predictBackDate = dataList[i].predictBackDate.slice(0, 16)
      // end 归还时间
      l.push(dataList[i])
    }
